...
type ReportData = {
  ...
  description: string;
  created_at: string;
  admin_notes: string | null; // Tambahan field dari Laravel
};
...
export default function TrackingPage() {
  ...
  const handleSearch = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Request GET ke Endpoint Laravel yang baru kita buat
      const response = await fetch(
        `http://127.0.0.1:8000/api/reports/${searchCode}`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok && result.success) {
        ...
        setErrorMsg(
          result.message ||
            "Tiket tidak ditemukan. Pastikan kode yang Anda masukkan benar.",
        );
      }
    } catch (error) {
      console.error(error);
      setErrorMsg("Koneksi gagal. Tidak dapat terhubung ke server Bawaslu.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950 transition-colors duration-500">
      ...
      <main className="flex-1 py-12 px-4 flex items-start justify-center">
        <div className="w-full max-w-2xl mt-10">
          ...
          <Card className="shadow-xl border-t-4 border-t-red-600 dark:border-t-red-500 mb-8 dark:bg-slate-900 dark:border-slate-800 transition-colors duration-300">
            <CardHeader className="border-b dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 pb-6">
              ...
              <CardDescription className="dark:text-slate-400">
                Masukkan Nomor Tiket (contoh: BWS-TGL-2026-XXXX) yang Anda
                dapatkan saat melapor.
              </CardDescription>
            </CardHeader>
          ...
          {/* HASIL PENCARIAN (REAL DARI DATABASE) */}
          {reportData && (
            <Card className="shadow-lg overflow-hidden border-2 border-slate-200 dark:border-slate-700 dark:bg-slate-900 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="bg-slate-100 dark:bg-slate-800/50 p-4 border-b dark:border-slate-700 flex items-center justify-between">
                <h3 className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Search className="w-4 h-4" /> Hasil Pencarian Database
                </h3>
              </div>
              <CardContent className="p-6 sm:p-8">
                ...
                {/* Pesan Admin (Muncul Jika Ada) */}
                {reportData.admin_notes && (
                  <div className="mb-8 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-5 rounded-xl flex gap-4 items-start">
                    <MessageSquareWarning className="w-6 h-6 text-blue-600 dark:text-blue-400 shrink-0 mt-0.5" />
                    <div>
                      <h4 className="text-sm font-bold text-blue-800 dark:text-blue-300 uppercase tracking-wider mb-1">
                        Pesan dari Bawaslu:
                      </h4>
